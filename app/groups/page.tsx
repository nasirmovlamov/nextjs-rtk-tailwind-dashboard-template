...
import { groupsApi } from '../redux/apis/GroupsApi';

export default function Groups() {
  const [getGroups, { data: response, isLoading: isLoadingGetGroups }] =
    groupsApi.useLazyGetGroupsQuery();
  ...
  return (
    <div className="relative flex flex-col w-full overflow-auto text-white bg-[#2b353dcd] shadow-md rounded-xl bg-clip-border">
      <TableSkeleton isLoading={isLoadingGetGroups} rows={10}>
          ...
          <tbody className="text-gray-300 font-thin">
            {response?.data?.data?.map((item) => (
              <tr key={item?.id}>
                ...
                <td className="p-4 border-b border-gray-600">
                  <p className="block">{item?.groupSerialNumber}</p>
                </td>
                {/* <td className="p-4 border-b border-gray-600">
